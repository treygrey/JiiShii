 ...
 */
export function applyPhoneCommand(runner, command) {
  ...
  switch (command.type) {
    ...
    case "phoneNotify":
      ...
      if (!runner.reconstructing) {
        runner.activeRenderer?.showPhoneToast?.(phone.notifications.at(-1), {
          onSelect: () => runner.openPhoneApp(command.app)
        });
      }
      runner.updatePhoneCheckpointState();
      runner.advanceCommand();
 ...
 * @returns {boolean} True when handled.
 */
export function applyGalleryCommand(runner, command) {
  ...
  if (command.type === "saveGalleryImage") {
    ...
    runner.projectSurface("gallery", { instant: runner.reconstructing });
    runner.updatePhoneCheckpointState();
    runner.advanceCommand();
    return true;
  }
 ...
 * @returns {boolean} True when handled.
 */
export function applySocialCommand(runner, command) {
  ...
  if (command.type === "socialPost") {
    ...
    if (command.notify) {
      const notification = addPhoneNotification(runner.state.visuals.phone, "social", {
      ...
      if (!runner.reconstructing) {
        runner.activeRenderer?.showPhoneToast?.(notification, {
          onSelect: () => runner.openPhoneApp("social")
        });
      }
    }
    runner.projectSurface("social", { instant: runner.reconstructing });
    runner.updatePhoneCheckpointState();
    runner.advanceCommand();
    return true;
  }
